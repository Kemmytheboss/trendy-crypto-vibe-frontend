...
function LiveDashboard({ compact = false }) {
  ...
  useEffect(() => {
    loadKlines(symbol);
    // rotate symbol when not compact
    let rot;
    if (!compact) {
      rot = setInterval(async () => {
        try {
          ...
          const top = tickers.filter(t => t.symbol.endsWith("USDT")).slice(0, 20);
  ...
  return (
    <div style={{ width: "100%", height: compact ? 180 : 320 }}>
      <h4 style={{ margin: "6px 0" }}>{compact ? "BTC 7-day" : `Live chart: ${symbol}`}</h4>
